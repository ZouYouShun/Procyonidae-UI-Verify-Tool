...
import Size from '../common/screenshot-size.component';
...
export default class Mosaic extends Action {
  ...
  static icon = 'screenshot-icon-mosaic';
  ...
  mousemove = (
    ...
    let lastTile =
      this.mosaic.history[0].tiles[this.mosaic.history[0].tiles.length - 1];
  ...
  mouseup = (e: MouseEvent, { el, ctx, context, setContext }: IActionProps) => {
    if (this.mosaic) {
      this.mosaic = null;
    }
  };

  draw = (ctx: CanvasRenderingContext2D, action: IHistory) => {
    const { tiles } = action;
  ...
  private getColor(x: number, y: number, size: number) {
